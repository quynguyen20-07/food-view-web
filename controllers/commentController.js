...
const commentController = {
  createComment: async (req, res) => {
    try {
      ...
      if (reply) {
        ...
        const repCmt = await Comments.findById(reply)
        if (!repCmt) return res.status(400).json({ message: "this comment does not exist" })
      }
  ...
  dislikeComment: async (req, res) => {
    try {
      ...
      res.json({ message: "Disliked this comment!" })
  ...
  deleteComment: async (req, res) => {
    try {
      ...
      const delComment = await Comments.findOneAndDelete({
        _id: req.params.id,
        $or: [
          { user: req.user._id },
      ...
      await Posts.findOneAndUpdate({ _id: delComment.postId }, {
